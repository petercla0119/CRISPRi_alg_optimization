...
**Objective**: The goal of the notebook is to use edgeR on the STMN2 screen.
Previous attempts to generate plots from the STMN2 gene file had noise plateaued.
Reminder that the MAGeCK output data is on the Desktop, whereas scripts are located
...
# NOTE: Why are there so many NA values in High_rep3? Like 78 NAs versus 16 in other samples. - higher events per second in high/low rep 3 - FACS device has harder time collecting high fluorescence events
...
-   `clean_raw_counts` is a df of raw count values without any 0s/NAs
...
count_matrix <- clean_and_process_counts_for_dge(raw_counts)
...
filtermanual_keep <- rowSums(cpm(dge$counts)>50)>=2
...
barplot(colSums(dge$counts), 
        ...
        main="Counts per index", sub = "With CPM > 50 in at least 2 samples",
        col=cols, cedge.names=0.5, cedge.adgeis=0.8)
barplot(rowSums(dge$counts), las = 2, 
        main = "Counts per sgRNA", sub = "With CPM > 50 in at least 2 samples",
